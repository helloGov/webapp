// TODO: Remove this file if/when we are truly using JS modules properly

require('./admin/admin.component');
require('./analytics.controller');
require('./campaigns-list/campaigns-list.component');
require('./create/create-form.component');
require('./matchInput.directive');
require('./account/signup.controller');
require('./account/login.controller');
require('./account/settings.component');
require('./visitor.controller');
require('./account/forgotPassword.component');
require('./account/resetPassword.component');
